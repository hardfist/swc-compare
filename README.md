...
GitHub Actions 会在 Node.js 20/24 上分别跑 500 模块和 256 KiB 大文件。共享 runner 上的数字只用于验证版本锁定、两种 loader 可构建、结果 schema、bundle hash 和运行时 checksum，不设置性能阈值；耗时、RSS、比值及正确性信息会写入 Job Summary，原始 JSON 与 Markdown summary 作为 artifact 保存 14 天。
